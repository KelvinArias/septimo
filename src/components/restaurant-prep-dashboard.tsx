...
import type { AppView, InventoryCategory, InventoryItem, Task } from "@/types";
...
export function RestaurantPrepDashboard() {
  ...
  useEffect(() => {
    async function loadData() {
      try {
        const [inventoryItems, loadedTasks] = await Promise.all([
          fetchInventoryItems(),
          fetchTasks(),
        ]);
  ...
  const filteredItems = filterInventoryItems(items, { search, category, lowOnly });
...
function getSubtitle(
  ...
  if (view === "tasks") {
    return `${pendingCount} pending - ${generatedTaskCount} from low stock`;
  }
  ...
  return `${itemCount} items - ${lowStockCount} low stock`;
}
